...
const controller = {
  deleteProject: (req, res) => {
    ...
    Project.findById(id, { thumbnail: true })
      .then(result => {
        if (result)
          Project.deleteOne({ _id: id }, error => {
            if (error) res.status(500).json(MESSAGES.DEFAULT_ERROR)
            else {
      ...
      .catch(error => {
        if (error.name == 'CastError') res.status(400).json(MESSAGES.CAST_ERROR)
        else res.status(500).json(MESSAGES.DEFAULT_ERROR)
      })
  },
  getProjects: (req, res) => {
  ...
  getProjectById: (req, res) => {
    ...
    Project.findById(id)
      .then(result => {
        ...
        else res.status(404).json(MESSAGES.NOT_FOUND)
      })
      .catch(error => {
        if (error.name == 'CastError') res.status(400).json(MESSAGES.CAST_ERROR)
        else res.status(500).json(MESSAGES.DEFAULT_ERROR)
  ...
  postProject: (req, res) => {
    ...
    if (typeof file != 'undefined' && file != null) saveThumbnail(data, file)
    else {
    ...
    project
      ...
      .catch(error => {
        ...
        if (error.name == 'ValidationError') res.status(422).json(errorMessage)
        else res.status(500).json(MESSAGES.DEFAULT_ERROR)
      })
  },
  patchProject: (req, res) => {
    ...
    if (patchThumbnail) saveThumbnail(data, file)

    Project.findOneAndUpdate({ _id: id }, data)
      .then(result => {
        if (patchThumbnail) {
          if (oldThumbnail === false) return
